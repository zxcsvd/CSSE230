...
//		Boolean find = true;
	...
	public class BinaryNode {
		...
		public int setHeight (BinaryNode current){
			if(current.left.data!=null && current.right.data!=null){
				this.height = Math.max(this.setHeight(current.left),this.setHeight(current.right))+1;
...
//				return 0;
				...
				this.height = this.setHeight(current.left)+1;
//				return this.getSumOfHeights(current.left)+1 ;
			}
			else if(current.left.data == null && current.right.data !=null){
				this.height = this.setHeight(current.right)+1;
...
//				this.height = this.getSumOfHeights(current.right)+1;
		...
		public int find(BinaryNode current, T target){
			...
			if(current.left.data!=null && current.right.data!=null){
				return this.setHeight(current.left) + this.setHeight(current.right);
			}
			else if(current.left.data== null && current.right.data == null){
				return 0;
			}
			else if(current.left.data != null && current.right.data == null){
				return this.setHeight(current.left);
			}
			else if(current.left.data == null && current.right.data !=null){
				return this.setHeight(current.right);
			}
			return 0;
		}
