...
public class ThreadedBinarySearchTree<T extends Comparable<? super T>> {
	...
	private void insert(T x, ThreadedBinaryNode<T> t) throws DuplicateItem {
		...
		if(x.compareTo(t.element)>0){
			if( t.isRightThread == true){
				t.isRightThread = false;
				ThreadedBinaryNode temp = t.right;
				t.right = new ThreadedBinaryNode<T>(x);
				t.right.left = t;
				t.right.right = temp;
			}
			else{
			...
			if(t.isLeftThread == true){
				t.isLeftThread = false;
				ThreadedBinaryNode temp = t.left;
				t.left = new ThreadedBinaryNode<T>(x);
				t.left.right = t;
				t.left.left = temp;
			}
			else{
...
//		 System.out.println("In insert(" + x + t + ")");
	...
	public String toStringInOrder() {
		...
		ThreadedBinaryNode current = t;
		boolean goFromThread = current.isRightThread;
		while(current != null){
			if(current.isRightThread){
				if(goFromThread){
					...
					result.append(current+"\n");
					goFromThread = current.isRightThread;
					current = current.right;
				}
			}
			else{
	...
	public String toStringInReverseOrder() {
		...
		ThreadedBinaryNode current = t;
		boolean goFromThread = current.isLeftThread;
		while(current != null){
			if(current.isLeftThread){
				if(goFromThread){
					...
					result.append(current+"\n");
					goFromThread = current.isLeftThread;
					current = current.left;
				}
			}
			else{
		...
		/*
		 * TODO: Add code here that starts at t and appends each node's text
		 * to result in the appropriate order.
		 */
