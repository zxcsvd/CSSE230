...
public class AdderMain {
	...
	public static void main(String[] args) {
		...
		ArrayList<Boolean> pressed = new ArrayList<Boolean>();
		pressed.add(false);
		pressed.add(false);
		...
		buttonPanel.setLayout(new GridLayout(4, 0, 5, 5));
		...
		for (int row = 0; row < 3; row++) {
			for (int col = 0; col < 3; col++) {
				...
				int num = 7 - 3*row + col;
				NumButtonListener numListener = new NumButtonListener(display, num, pressed);
		...
		for (String s : remainingButtonLabels) {
			...
			AddListener addListener = new AddListener(display,pressed);
			if(s.equals("0")){
				NumButtonListener numListener = new NumButtonListener(display, 0, pressed);
				b.addActionListener(numListener);
			}
			
			if(s.equals("+")){
				b.addActionListener(addListener);
			}
			
			if(s.equals("C")){
				CButtonListener cListener = new CButtonListener(display, pressed);
